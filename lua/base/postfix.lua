...
local function get_fix(fixes, line)
	...
	for i, match in ipairs(target_match) do
		local escaped_match = match:gsub("%%", "%%%0")
		parsed = parsed:gsub("%$" .. i, escaped_match)
	end
...
function M.setup(opts)
	...
	vim.keymap.set("i", "<C-j>", fix_postfix, { desc = "Postfixer: Apply postfix completion" })
end
